		...
		while( getline(&line, &bufsize, f) != -1 )
		{
			...
			if ( (sscanf( lowerline, "0x%x 0x%x iso8859-%d", &tsid, &onid, &encoding ) == 3 )
				...
				m_TransponderDefaultMapping[(tsid<<16)|onid]=0;
					...
					&& encoding == 936)
			        ||((sscanf( lowerline, "0x%x 0x%x %s", &tsid, &onid, codetemp ) == 3 )
					&& strcasecmp(codetemp,"gbk")==0)
			        ||((sscanf( lowerline, "%d %d %s", &tsid, &onid, codetemp ) == 3 )
					&& strcasecmp(codetemp,"gbk")==0)
				 )
			...
			else if ( ((sscanf( lowerline, "0x%x 0x%x %s", &tsid, &onid, codetemp ) == 3 )
						&& (strcasecmp(codetemp, "utf8")==0 || strcasecmp(codetemp, "utf-8")==0 ) )
					||((sscanf( lowerline, "%d %d %s", &tsid, &onid, codetemp ) == 3 )
						&&  (strcasecmp(codetemp, "utf8")==0 || strcasecmp(codetemp, "utf-8")==0 ) )
				 )
				m_TransponderDefaultMapping[(tsid<<16)|onid]=UTF8_ENCODING;
			else if ( ((sscanf( lowerline, "0x%x 0x%x %s", &tsid, &onid, codetemp ) == 3 )
						&& strcasecmp(codetemp, "utf16be")==0)
					||((sscanf( lowerline, "%d %d %s", &tsid, &onid, codetemp ) == 3 )
						&& strcasecmp(codetemp, "utf16be")==0)
				 )
				m_TransponderDefaultMapping[(tsid<<16)|onid]=UTF16BE_ENCODING;
			else if ( ((sscanf( lowerline, "0x%x 0x%x %s", &tsid, &onid, codetemp ) == 3 )
						&& strcasecmp(codetemp, "utf16le")==0)
					||((sscanf( lowerline, "%d %d %s", &tsid, &onid, codetemp ) == 3 )
						&& strcasecmp(codetemp, "utf16le")==0)
				 )
			...
			else if ( sscanf( lowerline, "%s %s", countrycode, codetemp ) == 2 &&
				  strcasecmp(codetemp, "utf8")==0)
			{
			   if ( countrycode[0] != '*' ){
				countrycode[0]=toupper(countrycode[0]);
				countrycode[1]=toupper(countrycode[1]);
				...
				defaultEncodingTable=UTF8_ENCODING;
				  ...
				  (strcasecmp(codetemp, "utf16be")==0 || strcasecmp(codetemp, "unicode")==0) )
			{
			   if ( countrycode[0] != '*' ){
				countrycode[0]=toupper(countrycode[0]);
				countrycode[1]=toupper(countrycode[1]);
